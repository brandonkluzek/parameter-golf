...
def main() -> None:
    ...
    run_summary: dict[str, object] = {
        ...
        "trainer": {
            ...
            "eval_ntk_alpha": args.eval_ntk_alpha,
    ...
    if master_process:
        ...
        run_summary["artifacts"].update(
            {
                "export_label": "int8",
                "compressor_label": "zstd" if args.use_zstd and zstd_mod is not None else "zlib",
                "export_model_bytes": quant_file_bytes,
                "export_payload_bytes": quant_stats["int8_payload_bytes"],
        ...
        emit_metric(
            "artifact",
            export_label="int8",
            compressor_label=run_summary["artifacts"]["compressor_label"],
            export_model_bytes=quant_file_bytes,
            export_payload_bytes=quant_stats["int8_payload_bytes"],
            export_total_bytes=quant_file_bytes + code_bytes,
            payload_ratio=ratio,
    ...
    log0(f"final_int8_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{eval_time_ms:.0f}ms")
    ...
    if args.eval_stride > 0:
        ...
        run_summary["metrics"].update(
            {
                "final_sliding_window_val_loss": sw_val_loss,
                "final_sliding_window_val_bpb": sw_val_bpb,
                "final_sliding_window_eval_time_ms": round(1000.0 * (time.perf_counter() - t_slide), 3),
            }
        )
        emit_metric(
            "final_sliding",
